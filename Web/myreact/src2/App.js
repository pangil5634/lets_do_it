...
function App() {
    ...
    // useEffect를 사용하여 컴포넌트가 마운트되었을 때 데이터를 가져옴 *** useEffect 함수는 컴포넌트가 렌더링 될 때마다 특정
    // 작업을 실행할 수 있도록 하는 Hook
    useEffect(() => {
        // 데이터를 가져오는 구문 async 함수를 만들어서 데이터를 가져오는 로직을 넣음
        const fetchData = async () => {
            // posts라는 컬렉션을 참조 컬렉션의 참조를 만들어서 데이터를 가져옴
    ...
    useEffect(() => {
        // onAuthStateChanged 함수를 사용하여 Firebase Authentication의 상태 변화를 감지한다. 이 함수는 사용자
        // 로그인 또는 로그아웃 상태가 변경될 때 호출된다.
